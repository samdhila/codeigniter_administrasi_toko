$(document).ready(function() {
	...
	.DataTable({
		...
		"sAjaxSource": base_url+'admin/c_supplier/get_json_data',			
		"bPaginate": true,		
		select: {
			style: 'single'
		}
...
function rem(frm,id){
	bootbox.confirm("Anda yakin akan menghapus data ini?", function(result) {						
		if(result) {
			window.location.href='c_supplier/delete_data/'+frm+'/'+id;
		}
	});
}
...
$('#btn_save').on('click',function(){
	...
	if(name !=''){
		$.ajax({
			type : "POST",
			url	 : base_url+'admin/c_supplier/add_data',
			dataType : "JSON",
...
function update(frm,id){
	$.ajax({
		url: base_url+'admin/c_supplier/cari_rec',
		method: 'GET',
		data: { 
			id: id,
			frm: frm

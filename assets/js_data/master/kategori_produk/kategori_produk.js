$(document).ready(function() {
	...
	.DataTable({
		...
		"sAjaxSource": base_url+'admin/c_kategori_produk/get_json_data',
		"bPaginate": true,
		select: {
			style: 'single'
		}
...
function rem(frm,id){
	bootbox.confirm("Anda yakin akan menghapus data ini?", function(result) {
		if(result) {
			window.location.href='c_kategori_produk/delete_data/'+frm+'/'+id;
		}
	});
}
...
$('#btn_save_product').on('click',function(){
	var id = $('#fld1').val();
	var category = $('#fld2').val();
	var parent = $('#fld3').val();
	if(category !="" && parent !=''){
		$.ajax({
			type : "POST",
			url  : base_url+'admin/c_kategori_produk/add_data',
			dataType : "JSON",
			data : {id:id, category:category, parent:parent},
...
function update(frm,id){
	$.ajax({
		url: base_url+'admin/c_kategori_produk/cari_rec',
		method: 'GET',
		data: {
			id: id,
			frm: frm

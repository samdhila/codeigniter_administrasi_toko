$(document).ready(function() {
	...
	.DataTable({
		...
		"sAjaxSource": base_url+'admin/c_unit/get_json_data',			
		"bPaginate": true,		
		select: {
			style: 'single'
		}
...
function rem(frm,id){
	bootbox.confirm("Anda yakin akan menghapus data ini?", function(result) {						
		if(result) {
			window.location.href='c_unit/delete_data/'+frm+'/'+id;
		}
	});
}
...
function update(frm,id){
	$.ajax({
		url: base_url+'admin/c_unit/cari_rec',
		method: 'GET',
		data: { 
			id: id,
			frm: frm
...
$('#btn_save_unit').on('click',function() {
	...
	if(name !=''){
		$.ajax({
			type : "POST",
			url	 : base_url+'admin/c_unit/add_unit',
			dataType : "JSON",

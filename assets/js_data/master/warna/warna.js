$(document).ready(function() {
	...
	.DataTable({
		...
		"sAjaxSource": base_url+'admin/c_warna/get_json_data',			
		"bPaginate": true,		
		select: {
			style: 'single'
		}
...
function rem(frm,id){
	bootbox.confirm("Anda yakin akan menghapus data ini?", function(result) {						
		if(result) {
			window.location.href='c_warna/delete_row/'+frm+'/'+id;
		}
	});
}
$('#btn_save_warna').on('click',function() {
	var id = $('#fld1').val();
	var name = $('#fld2').val();
	if(name !=''){
		$.ajax({
			type : "POST",
			url	 : base_url+'admin/c_warna/add_data',
			dataType : "JSON",
			data : {id:id, name:name},
			success: function(data){
				console.log(data);
				if(data.status == 'terdaftar'){
					alert('Warna sudah terdaftar..!!');
				}
...
function update(frm,id){
	$.ajax({
		url: base_url+'admin/c_warna/cari_rec',
		method: 'GET',
		data: { 
			id: id,
			frm: frm

$(document).ready(function() {
	...
	.DataTable({
		...
		"sAjaxSource": base_url+'admin/c_outlet/get_json_data',
		"bPaginate": true,
		select: {
			style: 'single'
		}
...
function rem(frm,id){
	bootbox.confirm("Anda yakin akan menghapus data ini?", function(result) {
		if(result) {
			window.location.href='c_outlet/delete_data/'+frm+'/'+id;
		}
	});
}
...
$('#btn_save').on('click',function(){
	...
	var alamat = $('#fld3').val();
	var area = $('#fld4').val();
	var provinsi = $('#fld5').val();
	var kota = $('#fld6').val();
	var pos = $('#fld7').val();
	var telepon = $('#fld8').val();
	var latitude = $('#fld9').val();
	var longitude = $('#fld10').val();
	if(name !="" && alamat !='' && area !='' && provinsi !='' && kota !='' && pos !='' && telepon !='' && latitude !='' && longitude !=''){
		$.ajax({
			type : "POST",
			url  : base_url+'admin/c_outlet/add_data',
			dataType : "JSON",
			data : {id:id, name:name, alamat:alamat, area:area, provinsi:provinsi, kota:kota,
			pos:pos, telepon:telepon, latitude:latitude, longitude:longitude},
			success: function(data){
				if(data.status == 'terdaftar'){
					alert('Kantor sudah terdaftar..!!');
				}
...
function update(frm,id){
	$.ajax({
		url: base_url+'admin/c_outlet/cari_rec',
		method: 'GET',
		data: {
			id: id,
			frm: frm

...
class C_pemesanan_3 extends CI_Controller {
	public function __construct(){    
		...
		$this->load->model('m_pemesanan_3','salesorder');
	...
	public function index(){
		...
		$this->template->useAsset()->setJs('transaksi/pemesanan',array('pemesanan_3'));
	...
	function get_data_pemesanan(){
		...
		foreach ($list as $salesorder) {
			...
			if($salesorder->status == 3){
				$status = '<span class="label label-sm label-primary arrowed-in">Done</span>';
			}

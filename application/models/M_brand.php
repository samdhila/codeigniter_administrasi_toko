...
class M_brand extends CI_Model{
	...
	function save_data($name){
		$hasil= $this->db->query("SELECT * FROM brand WHERE name = '$name'");
		$result = $hasil->result_array();
		$count = count($result);
		if(empty($count)){
			$hasil= $this->db->query("INSERT INTO brand (name)VALUES('$name')");
		}
		else{
	...
	function update_data($id,$name){
		$hasil=$this->db->query("UPDATE brand SET name='$name' WHERE id='$id'");
		return $hasil;
	}
}
